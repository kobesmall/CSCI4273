...
int delete(int sockfd,char *buf, struct sockaddr * clientaddr, socklen_t clientlen){
  char *fil = buf + 7;
  ...
  a = remove(fil);
  if(a==0){
  sendto(sockfd,suc, sizeof(suc), 0, clientaddr, clientlen);
  }
  if(a!=0){
    sendto(sockfd,fail, sizeof(fail), 0, clientaddr, clientlen);
  }
...
void ls_(int sockfd, struct sockaddr * clientaddr, socklen_t clientlen){
  ...
  struct dirent *myfile;
   mydir = opendir("./");
    ...
    sendto(sockfd, myfile->d_name, sizeof(myfile->d_name), 0, clientaddr, clientlen);
